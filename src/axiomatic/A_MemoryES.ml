...
let freeze (es, exns, jst, ppo, rmw, data, addr, ctrl, casdep, jst_hist) : t =
  ...
  (es, new_exns, empty_justification, ppo, rmw, data, addr, ctrl, casdep, jst@jst_hist)
...
(* Add something to the start of ppo *)
let prefix_ppo ppo es ((i, _) as event) =
  let ppo' = List.map (fun e -> (i, id e)) (sloc_events event (events es)) in
  let ppo' = ppo' @ List.map (fun e -> (i, id e)) (List.filter (fun e -> is_lock e || is_unlock e) (events es)) in
  ppo <|> ppo'

let pre_compose regs psi gamma c (es1, exns, jst, ppo, rmw, data, addr, ctrl, casdep, jst_hist) : event -> t = function
  | (i, R _) as r ->
     let es = EventStructure.prefix_event es1 r in
     let ppo = prefix_ppo ppo es1 r in
     ...
     let lb_reads =
       List.filter (fun r' ->
             not (List.exists
                  (fun b -> List.mem (id r, id b) ppo && List.mem (id b, id r') ppo)
                  (events es)
               )
         ) iso_reads
     in
     ...
     (es, exns, jst, ppo, rmw, data, addr, ctrl, casdep, jst_hist)

  | (i, W _) as w ->
     let es = EventStructure.prefix_event es1 w in
     let ppo = prefix_ppo ppo es1 w in
     ...
     let sb_reads =
       List.filter (fun r' ->
             not (List.exists
                  (fun b -> List.mem (id w, id b) ppo && List.mem (id b, id r') ppo)
                  (events es)
               )
         ) iso_reads
     in
     ...
     let jst'' = List.map (fun (c, c', e) ->
                     if List.exists (fun e' -> List.mem (id w, id e') ppo) c
                     then (w :: c', e)
                     else (c', e)
                   ) jst' in
     ...
     (es, exns, ind_just <|> jst'', ppo, rmw, data, addr, ctrl, casdep, jst_hist)

  | (i, U) | (i, L) as l ->
     let den = es1, exns, jst, ppo, rmw, data, addr, ctrl, casdep, jst_hist in
     let es = EventStructure.prefix_event es1 l in
     let ppo = prefix_ppo ppo es1 l in
     let (_es1', exn1', _jst1', _ppo1', _rmw, _data, _addr, _ctrl, _casdep, jst_hist)  = freeze den in
     let exns =
       List.map (fun (evs, _rf, dep, _lck, co, _) ->
           ((i :: evs), [], dep, [], co, [])
         ) exn1'
     in
     (es, exns, empty_justification, ppo, rmw, data, addr, ctrl, casdep, jst_hist)
...
let restricted_uc (id, ev) ppo bob js =
  match ev with
  | R _ ->
     let ppo_restricted =
       List.filter (fun (c, _e, _, _, _, _, _, _, _, _) ->
           let idl = List.map fst c in
           (not(List.exists (fun (ida, idb) -> id = ida && List.mem idb idl) ppo)) &&
             (* The paper doesn't include bob *)
             not(List.exists (fun (ida, idb) -> id = ida && List.mem idb idl) bob)
         ) js
     in
     ppo_restricted
...
let ctx_iso bob bob' ppo ppo' c c' =
  let iso_candidates = Util.n_cartesian_product @@ List.map (fun e -> List.map (fun e' -> (e, e')) c') c in
  let idl_c = List.map fst c in
  let idl_c' = List.map fst c' in
  let ppo = List.filter (fun (a, b) -> List.mem a idl_c && List.mem b idl_c) ppo in
  let ppo' = List.filter (fun (a, b) -> List.mem a idl_c' && List.mem b idl_c') ppo' in
  let bob = List.filter (fun (a, b) -> List.mem a idl_c && List.mem b idl_c) bob in
  let bob' = List.filter (fun (a, b) -> List.mem a idl_c' && List.mem b idl_c') bob' in
  let isos =
    List.exists (fun iso ->
        List.for_all (fun (a, b) -> event_isomorphism a b) iso
        && (let idl_iso = List.map (fun ((i, _), (i', _)) -> (i, i')) iso in ap_isomorphism idl_iso ppo = ppo')
        && (let idl_iso = List.map (fun ((i, _), (i', _)) -> (i, i')) iso in ap_isomorphism idl_iso bob = bob')
...
let lifting t continuations =
  let _les, _exns, jsts, _ppos, _rmw, _data, _addr, _ctrl, _casdep, _jst_hist = t in
  let xjustifications =
    Util.n_cartesian_product (
        List.map (fun (ev, (les, _, j, ppo, rmw, data, addr, ctrl, casdep, jst_hist)) ->
            List.map (fun (c, e) -> (c, e, ev, les, ppo, rmw, data, addr, ctrl, casdep, jst_hist)) j
          ) continuations)
  in
  ...
  let xjustifications' =
    List.filter (fun js ->
        let (_c, x, _ev, _les, _ppo, _rmw, _data, _addr, _ctrl, _casdep, _jst_hist) = List.hd js in
        List.for_all (fun (_c', y, _ev, _les, _ppo, _rmw, _data, _addr, _ctrl, _casdep, _jst_hist) -> event_isomorphism x y) js
      ) xjustifications
  in
  ...
  let xjustifications'' =
    List.fold_right (fun js acc ->
        let uc_ctxs =
          List.map (fun (c, e, ev, les, ppo, rmw, data, addr, ctrl, casdep, jst_hist) ->
              let c = c <-> [ev] in
              let ctxs = upclosure les ppo c in
              let edges = List.map (fun c -> c, e, les, ppo, rmw, data, addr, ctrl, casdep, jst_hist) ctxs in
              let bob = Imm.imm_bob les in
              (*let ppo = Imm.imm_ppo les (order les) [] (ctrl <|> data <|> addr) casdep in*)
              restricted_uc ev ppo bob edges
            ) js
        in
        ...
        let xuc_ctxs =
          List.filter (fun js ->
              match js with
                (c, _x, les, ppo, _, _data, _addr, _ctrl, _casdep, _) :: js ->
                let bob = Imm.imm_bob les in
                (*let ppo = Imm.imm_ppo les (order les) [] (ctrl <|> data <|> addr) casdep in*)
                List.for_all (fun (c', _y, les', ppo', _, _data', _addr', _ctrl', _casdep', _) ->
                    let bob' = Imm.imm_bob les' in
                    (*let ppo' = Imm.imm_ppo les (order les') [] (ctrl' <|> data' <|> addr') casdep' in*)
                    ctx_iso bob bob' ppo ppo' c c'
                  ) js
              | _ -> true
            ) xuc_ctxs
        in
  ...
  jsts <|> List.map (fun (jst, e, _, _, _, _, _, _, _, _) -> jst, e) jsts'
...
let coproduct vs r g o p regs psi gamma c k : t =
  ...
  in
  let (les, exns, _jsts, ppos, rmw, data, addr, ctrl, casdep, jst_hist) as t =
    List.fold_right (fun (_, (es, exn, jst, ppo, rmw, data, addr, ctrl, casdep, jst_hist))
                       (ess, exns, jsts, ppos, rmws, datas, addrs, ctrls, casdeps, jst_hist') ->
        let ess = es_coproduct es ess in
        let exns = exn <|> exns in
        let jsts = jst <|> jsts in
        let ppos = ppo <|> ppos in
        let rmw = rmw <|> rmws in
        ...
        ess, exns, jsts, ppos, rmw, data, addr, ctrl, casdep, jst_hist
      ) continuations (empty_les, [], [], [], [], [], [], [], [], [])
  in


  let jsts = lifting t continuations in
  (les, exns, jsts, ppos, rmw, data, addr, ctrl, casdep, jst_hist)
...
let fadd (w_g, e, w_o, s) vs r g o p regs psi gamma c k : t =
  ...
  in
  let (les, exns, _jsts, ppos, rmw, data, addr, ctrl, casdep, jst_hist) as t =
    List.fold_right (fun (_, rmw', (es, exn, jst, ppo, rmw, data, addr, ctrl, casdep, jst_hist))
                       (ess, exns, jsts, ppos, rmws, datas, addrs, ctrls, casdeps, jst_hist') ->
        let ess = es_coproduct es ess in
        let exns = exn <|> exns in
        let jsts = jst <|> jsts in
...
let cas (w_g, e1, e2, w_o, s) vs r g o p regs psi gamma c k =
  ...
  in
  let (les, exns, _jsts, ppos, rmw, data, addr, ctrl, casdep, jst_hist) as t =
    List.fold_right (fun (_, rmw', cas', (es, exn, jst, ppo, rmw, data, addr, ctrl, casdep, jst_hist))
                       (ess, exns, jsts, ppos, rmws, datas, addrs, ctrls, casdeps, jst_hist') ->
        let ess = es_coproduct es ess in
        let exns = exn <|> exns in
        let jsts = jst <|> jsts in
...
let product d1 d2 =
  let (es1, exns1, _, ppo1, rmw1, data1, addr1, ctrl1, casdep1, jst_hist1) = freeze d1 in
  let (es2, exns2, _, ppo2, rmw2, data2, addr2, ctrl2, casdep2, jst_hist2) = freeze d2 in
  ...
  (es, exns, empty_justification, ppo1 <|> ppo2,
   rmw1 <|> rmw2, data1 <|> data2, addr1 <|> addr2,
   ctrl1 <|> ctrl2, casdep1 <|> casdep2, jst_hist1 @ jst_hist2)

let products (ds : t list) : t =
  let project_es (es, _, _, _, _, _, _, _, _, _) = es in
  let project_exns ((_, exns, _, _, _, _, _, _, _, _) : t) = exns in
  let project_ppo ((_, _, _, ppo, _, _, _, _, _, _) : t) = ppo in
  let project_jst_hist ((_, _, _, _, _, _, _, _, _, jst_hist) : t) = jst_hist in
  ...
  in
  let ppo = List.fold_right (<|>) (List.map project_ppo frozen) [] in
  let jst_hist = List.fold_right (@) (List.map project_jst_hist frozen) [] in
  let rmw, data, addr, ctrl, casdep =
    List.fold_right (fun (_, _, _, _, rmw, data, addr, ctrl, casdep, _)
...
let quantified_rf_lck ((les, exns, jst, ppo, rmw, data, addr, ctrl, casdep, jst_hist) : t) : t =
  ...
  let rf_candidates =
    ...
    (les, rf_candidates, jst, ppo, rmw, data, addr, ctrl, casdep, jst_hist)

let apply_axioms ((les, exns, jst, ppo, rmw, data, addr, ctrl, casdep, jst_hist) : t) : t =
  ...
  in
  (les, coherent_executions, jst, ppo, rmw, data, addr, ctrl, casdep, jst_hist)

let run_map m id =
  try m id with _ -> id

let join (es1, exns1, jst1, ppo1, rmw1, data1, addr1, ctrl1, casdep1, jst_hist1)
         (es2, exns2, _jst2, ppo2, rmw2, data2, addr2, ctrl2, casdep2, jst_hist2) : t =
  ...
  let ppo2 =
    List.flatten @@
      List.map (fun m ->
          let m = run_map m in
          List.map (fun (a, b) -> m a, m b) ppo2
        ) maps in
  let ppo = ppo1 <|> ppo2 in
...
let check closed outcome (les, exns, _, _, _, _, _, _, _, _) =
