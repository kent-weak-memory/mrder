...
let rec denote vs n p psi gamma k = function
  ...
  | Parallel (p1, p2) ->
     ...
     let k' p psi gamma = 
      if k p psi gamma = empty
      then products (List.map (denote vs n p psi gamma empty_continuation) ps)
      else join (products (List.map (denote vs n p psi gamma empty_continuation) ps)) (denote vs n p psi gamma k (Sequence (Lock, Unlock)))
     in
     ...
     A_MemoryES.freeze @@ k' p psi gamma
