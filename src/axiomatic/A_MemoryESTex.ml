...
let pp_jst fmt (_les, exns, jst, ppo, rmw, _, _, _, _, _) =
  ...
  List.iter (fun (ids, rf, dep, lck, co, _) ->
      ...
      let ppo_exn = List.filter (fun (a, b) -> List.mem a ids && List.mem b ids && notlock (a,b)) ppo in
      ...
      Format.fprintf fmt "& \\ppo = %s \\\\ \n" (string_of_reln ppo_exn);
...
let pp_tex' fmt (es, _, jst, _ppo, rmw, _, _, _, _, _) =
...
let pp_tex fmt es =
  ...
  let _, _, _, _, _, data, _, ctrl, _, _ = es in
  Format.fprintf fmt "$$data \\cup ctrl = \\{ %a \\}$$"
                 (pp_list (pp_pair pp_id)) (data <|> ctrl)


let pp_con_tex' fmt ((es, _, jst, _, rmw, _, _, _, _, _), con) =
...
let pp_exn_tex' fmt ((es, _, jst, _, _, _, _, _, _, _), (evs, rf, dep, lck, co, _)) =
  let pp_just fmt (c, ((id_e, _) as e)) =
    List.iter (fun ((id_x, _) as x) ->
