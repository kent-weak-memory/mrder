...
let test_equal_set _ =
  OUnit2.assert_bool "sets equal" (
    equal_set (=) [3;4;5] [4;3;5] &&
    equal_set (equal_set (=)) [[3;4]] [[4;3]]
  );
  OUnit2.assert_bool "sets not equal" (not (equal_set (=) [3] []))

let test_sequence_rel _ =
  let r1 = [(1, 2); (2, 3); (4,5)] in
  let r2 = [(2,5)] in
  let expected = [(1, 5)] in
  OUnit2.assert_bool "sequence relation" (equal_set (=) expected (rel_sequence r1 r2))

let suite = OUnit2.(
    "relation tests" >:::
      [ "test set equality">:: test_equal_set
      ; "test sequence relation" >:: test_sequence_rel
      ]
  )
