...
let apply_axioms (les, exns, jst, ppo, rmw, data, addr, ctrl, casdep) =
  ...
  let atomics = List.filter (fun e -> not (is_nonatomic e)) (events les) in
  let atomic_ids = List.map fst atomics in

  let rs_c20 ids rf rmw =
    let atomic_writes = List.filter (fun w -> not (is_nonatomic w)) writes in
    let rs_a = rel_of_set (List.map fst atomic_writes) in
    let rs_b = rtc ids (seq rf rmw) in
    seq rs_a rs_b
  in

  let sc_events = filter_by_ordering [SC] (events les) in
  let sc_event_ids = List.map fst sc_events in
  ...
  let release_rc11 ids po rf rmw =
    ...
    seq release_a (rs_c20 ids rf rmw)
  in
  ...
  let strongly_hb_c20 ids po rf rmw =
    let sw = sw_rc11 ids po rf rmw in
    let hb = imm_hb_rc11 ids po rf rmw in
    let shb_a = po in
    let shb_b = (sw <&> (cross sc_event_ids sc_event_ids)) in
    let shb_c = seq (seq po hb) po in
    tc (shb_a <|> shb_b <|> shb_c)
  in
  
  let imm_fr rf co = seq (inv rf) co in

  let eco_c20 ids rf co =
    let eco_a = rf in
    let eco_b = co in
    let eco_c = imm_fr rf co in
    (cross atomic_ids atomic_ids) <&> tc (eco_a <|> eco_b <|> eco_c)
  in
...
(*    Format.fprintf fmt "hb = %a\n" (pp_list (pp_pair pp_id)) hb;
  ...
  in
  (les, exns, jst, ppo, rmw, data, addr, ctrl, casdep)
