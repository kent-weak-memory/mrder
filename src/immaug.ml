...
let apply_axioms (les, exns, jst, ppo, rmw, data, addr, ctrl, casdep, jst_hist) =
  ...
  let sw ids po rf =
    ...
    let sw_c = (rel_of_set r_acq_ids) <|> seq (order les) (rel_of_set acq_sc_fence_ids) in

    (* release ; (rfi ∪ poₗ?;rfe) ; ([Rᵃᶜ] ∪ po;[F^{⊑ac}] *)
        ...
        let co_cands =
          List.filter (fun co ->
              let r =  rf_completeness rf reads
                       ...
                       (* NOTE: This is the augmentation, replacing
                       addr and ctrl with our `dep' relation in the no
                       thin air axiom *)
                       && no_thin_air ids po co rf dep casdep
